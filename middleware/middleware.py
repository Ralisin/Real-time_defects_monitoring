...
def consume_q1_redis(redis_host='redis', redis_port=6379, channel='saturated-pixels', csv_path='results/saturated-pixels.csv', verbose=False):
...
def consume_q2_redis(redis_host='redis', redis_port=6379, db=0, channel='saturated-rank', csv_path='results/saturated-rank.csv', verbose=False):
...
def consume_q3_redis(redis_host='redis', redis_port=6379, db=0, channel='centroids', server_url=None, bench_id=None, csv_path='results/centroids.csv', verbose=False):
    ...
    def flush_buffer():
        ...
        with open(csv_path, mode='a', newline='') as csvfile:
            ...
            while next_expected_id in buffer:
                ...
                print_id, tile_id, saturated, centroids_str = row
                ...
                if server_url:
                    try:
                        # Parsing centroids
                        centroids = json.loads(centroids_str)
                        payload = {
                            "print_id": print_id,
                            "tile_id": int(tile_id),
                        ...
                        resp = session.post(
                            f"{server_url}/api/result/0/{bench_id}/{next_expected_id}",
                            json=payload
                        )
                        resp.raise_for_status()
                        logger.info(f"[consume_q3_redis] Posted result seq_id={next_expected_id} — Response: {resp.text}")

                    except Exception as e:
                        logger.error(f"[consume_q3_redis] Failed to post seq_id={next_expected_id}: {e}")
